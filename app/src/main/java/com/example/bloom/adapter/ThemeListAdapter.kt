...
import com.example.bloom.databinding.ThemeListItemBinding
import kotlinx.coroutines.CoroutineScope
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.launch

class ThemeListAdapter(private val context: Context) :
    ListAdapter<Media, ThemeListAdapter.ItemViewHolder>(DiffCallback) {
    // Provide a reference to the views for each data item
    // Complex data items may need more than one view per item, and
    // you provide access to all the views for a data item in a view holder.
    // Each data item is just an Affirmation object.
    class ItemViewHolder(val itemViewBinding: ThemeListItemBinding) :
